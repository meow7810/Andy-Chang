...
app/src/main/java/com/andychang/clauderi/
  ...
  capabilities/ActionsCapability.kt       send_message / set_alarm / set_timer / play_music / control_media / now_playing / navigate_to（Intent、媒體鍵、MediaSession）
...
4. **動作**：簡訊（開簡訊 App 填好、由你按送出）、鬧鐘、計時器、播放音樂（搜尋後交給預設音樂 App）、暫停／切歌（送媒體鍵給正在播的 App）、單曲／全部循環、隨機、查正在播什麼（走 MediaSession，需系統通知存取）、導航（Google Maps），全走系統 Intent。
